    ...
    for (int i = 0; i < _altura; i++) {
        for (int j = 0; j < _largura; j++) {
            if (i < sobra_y || j < sobra_x || i >= (_altura - sobra_y) || j >= (_largura - sobra_x)) {
                nova_matriz[i][j] = 0;  // Acessos independentes
            } else {
...
int *PBM_Img::identificar_objetos() {
    ...
    #pragma omp parallel for reduction(+:qtdPalavras) shared(ultimoMaxY, novaMatriz)
    for(int i = 0; i < _altura; i++) {
        int localUltimoMaxY = -1;
        ...
        #pragma omp critical
        {
            if(i == localUltimoMaxY) qtdLinhas++;
            if(localUltimoMaxY > ultimoMaxY) ultimoMaxY = localUltimoMaxY;
        }
    }
